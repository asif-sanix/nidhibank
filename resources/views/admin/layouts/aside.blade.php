         ...
         @php
            ...
            @foreach ($menus as $menu)
                @if(!$menu->childs->count() && Route::has("admin.".Str::slug($menu->slug, '-').".index"))
                    <li class="{{ request()->segment(2) == Str::slug($menu->slug, '-')?'active':''}} nav-item">
                        <a href="{{ route("admin.".Str::slug($menu->slug, '-').".index")}}">
                     ...
                     @foreach($menu->childs as $child)
                         @if(Route::has("admin.".Str::slug($child->slug, '-').".index"))
                            <li class="{{ ($child->slug == str_replace('-', '_', request()->segment(2)))?'active':'' }}">
                                <a class="menu-item" href="{{ route('admin.'.Str::slug($child->slug, '-').'.index')}}">{{ $child->name }}</a>
                            </li>
                         @endif
